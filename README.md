...
In this example, the `mcx` function takes three arguments: `m`, `c`, and `x`. The function body multiplies `m` and `x`, then adds `c` to the result. For convenience we can store this function in a variable, in this example `f`.
...
In this example, the values `3`, `2`, and `5` are passed to the function as `m`, `c` and `x`.

The result of the function is `17` i.e. `3` x `5` + `2`.

### Global Variables

In addition to function arguments, MONTY supports the use of global variables. These global variables can be accessed and modified from anywhere within a MONTY program.

### Mathematical Example

** THIS SEEMS REDUNDANT AND REPEATS THE SBOVE EXAMPLE. ALSO `c` DOESN'T HAVE A VALUE DEFINED. I THINK THESE TWO SECXTION CAN BE MERGED. **
...
C IS UNDEFINED IN THIS EXAMPLE
...
In MONTY, blocks of code enclosed within { } are not executed immediately. Like functions, they can be executed later
...
A variable's value can be accessed simply by referring to it by name
...
The language can also get the remainder of a previous division by using `/re`.
...
** THIS SECTION NEEDS WORK, I'LL EXPLAIN STRINGS BETTER SOON **
...
** NEEDS WORK, MONTY ONLY HAS LIMITED STRING OPERATIONS AT PRESENT **
...
** NEEDS WORK, HMMM... THESE EXAMPLES PROBABLY DON'T WORK **
...
** I THINK STRINGS AND ARRAYS NEED TO BE DISCUSSED TOGETHER AS MEMORY OPERATIONS, I'LL EXPLAIN BETTER SOON **
...
** == ONLY COMPARES EQUAL NUMERIC VALUES, STRING COMPARISON DOES NOT EXIST CURRENTLY **
...
THIS IS AN EXAMPLE OF A SWITCH, NEED TO EXPAND LATER
...
** NEEDS WORK, THERE ARE NO BUILT IN ITERATOR VARIABLES like `i`, ONLY GLOBAL VARIABLES AND ARGUMENTS **
...
WILL NEED TO CHECK THIS TO SEE THAT IT WORKS AS EXPECTED
...
1. This code defines a function that does factorial and stores it in a variable `F`
...
4. If it's not less than 2, it calculates the factorial recursively by multiplying the number n by the factorial of n-1. It calls the factorial function recursively using `F^`.
...
```
:n { %n 2 < { [0 1] } { [0 1] %n 2 - i= ( %array %i 1% %array %i 2% + %array /a array= i i 1 + = )^ %array } ?? } ; F=
```
